...
--docs
create index idx_docs_doc_numb on docs (doc_number) reverse; --необходим дл€ поиска документов по номеру (номер в виде <префикс>_<номер>)
--history_prices
--индексы дл€ получени€ истории цен по товару
...
--individuals
create bitmap index idx_individ_gender on individuals (gender);--значени€ только F или M
...
--loyprog_by_products
create index idx_loypr_prod_prod_loy_act_date on loyprog_by_products (id_product, id_loyprog, actual_date); --по продукту, программе ло€льности и дате актуальности (может быть несколько строчек, поэтому не уникальный)
create index idx_loypr_prod_prod_amount on loyprog_by_products (id_product, amount); --товар, продаваемый с наибольшей скидкой
--partners
--поиск юр.лиц по »ЌЌ, ќ√–Ќ, названию
create index idx_partners_inn on partners (inn);
create index idx_partners_ogrn on partners (ogrn);
create index idx_partners_name on partners (name);
--products
create index idx_products_art on products (article); --дл€ поиска по артиклу
create index idx_products_name on products (name); --дл€ поиска по названию
--products_by_spec
...
--specifications
create bitmap index idx_spec_is_search on specifications (is_search); --значени€ только Y или N
--users
create unique index idx_users_login on users (login); -- логин пользовател€ должен быть уникальным
--warehouse
create bitmap index  idx_wh_pickup on warehouse (pickup); --значени€ только Y или N
-----------------------------------------------------------------------
--—оздаем ограничени€ (здесь отражены дополнительные ограничени€. NOT NULL создавались вместе с пол€ми, их можно посмотреть в схеме Ѕƒ (об€зательные отмечены красной точкой) 
--users
ALTER TABLE USERS
